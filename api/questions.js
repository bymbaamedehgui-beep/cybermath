...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'POST') {
      const { text, topic, grade, correct, choices, hint } = req.body || {};
      if (!text || !correct) return res.status(400).json({ ok: false, error: 'Missing fields' });
      const r = await pool.query(
        'INSERT INTO questions (text,topic,grade,correct,choices,hint) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *',
        [text, topic, grade, correct, choices, hint ? JSON.stringify(hint) : null]
      );
