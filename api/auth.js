...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (action === 'register') {
      ...
      await pool.query(
        'INSERT INTO users (email,pass,first_name,last_name,grade,plan,xp,gems,hearts,streak,avatar) VALUES ($1,$2,$3,$4,$5,$6,0,340,5,0,$7)',
        [email, pass, firstName, lastName, grade, plan || 'free', 'default']
      );
      return res.json({ ok: true, user: {
        email, firstName, lastName, grade,
        plan: plan || 'free',
        xp: 0, gems: 340, hearts: 5, streak: 0, avatar: 'default'
      }});
    }
    ...
    // GET USER BY EMAIL (no password needed - for session refresh)
    if (action === 'getuser') {
