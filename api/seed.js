...
module.exports = async (req, res) => {
  ...
  try {
    // Seed нь зөвхөн INSERT биш: "DELETE FROM nodes WHERE grade='6' AND id<300" (дуудлага бүрд),
    // нэг удаагийн DELETE/UPDATE (ws_place, ws_subgroups), ws_settings sg_seeded_* дарж бичих,
    // админы устгасан node-уудыг дахин үүсгэх зэрэг байгаа өгөгдлийг өөрчилдөг тул хамгаална.
    ...
    try {
      ...
      // ── Нэг удаагийн: 8-р анги V БҮЛЭГ (Дараалал, функц) ──
      const cf8e = await pool.query(`SELECT sval FROM ws_settings WHERE skey='place_g8ch5_v1'`);
      if (!cf8e.rows.length) {
        let par8e = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='8-р анги' AND name LIKE 'V БҮЛЭГ%' AND parent_id IS NULL ORDER BY id LIMIT 1`);
        let pid8e;
        if (par8e.rows.length) { pid8e = Number(par8e.rows[0].id); }
        else {
          const mxP = await pool.query(`SELECT COALESCE(MAX(pos),0)+1 AS p FROM ws_subgroups WHERE grade='8-р анги'`);
          const insP = await pool.query(`INSERT INTO ws_subgroups (grade, name, pos, parent_id) VALUES ('8-р анги','V БҮЛЭГ. ДАРААЛАЛ, ФУНКЦ',$1,NULL) RETURNING id`, [mxP.rows[0].p]);
          pid8e = Number(insP.rows[0].id);
        }
        const subs8e = [
          { name: '5.1 Дараалал', slugs: ['daraalal-8.html'] },
          { name: '5.2 Арифметик прогресс', slugs: ['arifm-progress-1-8.html', 'arifm-progress-2-8.html'] },
          { name: '5.3 Шугаман функц, түүний график', slugs: ['shugaman-funkts-1-8.html', 'shugaman-funkts-2-8.html'] },
          { name: '5.4 Шулууны налалт, ШТС-ийн график', slugs: ['nalalt-shts-8.html'] },
          { name: '5.5 Урвуу пропорционал хамаарлын график', slugs: ['urvuu-grafik-8.html'] },
          { name: '5.6 y=ax² функц, түүний график', slugs: ['parabol-1-8.html', 'parabol-2-8.html'] }
        ];
        for (const sub of subs8e) {
          let sgr = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='8-р анги' AND name=$1`, [sub.name]);
          let sid;
          if (sgr.rows.length) { sid = Number(sgr.rows[0].id); }
          else {
            const mx = await pool.query(`SELECT COALESCE(MAX(pos),0)+1 AS p FROM ws_subgroups WHERE grade='8-р анги'`);
            const ins = await pool.query(`INSERT INTO ws_subgroups (grade, name, pos, parent_id) VALUES ('8-р анги',$1,$2,$3) RETURNING id`, [sub.name, mx.rows[0].p, pid8e]);
            sid = Number(ins.rows[0].id);
          }
          for (const slug of sub.slugs) {
            await pool.query(`INSERT INTO ws_place (grp, slug, kind) VALUES ($1,$2,'add') ON CONFLICT DO NOTHING`, ['sg:' + sid, slug]);
        ...
        await pool.query(`INSERT INTO ws_settings (skey, sval) VALUES ('place_g8ch5_v1','1') ON CONFLICT (skey) DO UPDATE SET sval='1'`);
      }
      // ── Нэг удаагийн: 8-р анги VI БҮЛЭГ (Өнцөг, дүрс, байгуулалт) ──
      const cf8f = await pool.query(`SELECT sval FROM ws_settings WHERE skey='place_g8ch6_v1'`);
      if (!cf8f.rows.length) {
        let par8f = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='8-р анги' AND name LIKE 'VI БҮЛЭГ%' AND parent_id IS NULL ORDER BY id LIMIT 1`);
        let pid8f;
        if (par8f.rows.length) { pid8f = Number(par8f.rows[0].id); }
        else {
          const mxP = await pool.query(`SELECT COALESCE(MAX(pos),0)+1 AS p FROM ws_subgroups WHERE grade='8-р анги'`);
          const insP = await pool.query(`INSERT INTO ws_subgroups (grade, name, pos, parent_id) VALUES ('8-р анги','VI БҮЛЭГ. ӨНЦӨГ, ДҮРС, БАЙГУУЛАЛТ',$1,NULL) RETURNING id`, [mxP.rows[0].p]);
          pid8f = Number(insP.rows[0].id);
        }
        const subs8f = [
          { name: '6.1 Олон өнцөгтийн дотоод ба гадаад өнцөг', slugs: ['olon-ongogt-1-8.html', 'olon-ongogt-2-8.html'] },
          { name: '6.2 Гурвалжны дундаж шугам, өндөр, медиан, биссектрис', slugs: ['gurvaljin-shugam-8.html'] },
          { name: '6.3 Пифагорын теорем', slugs: ['pifagor-1-8.html', 'pifagor-2-8.html'] },
          { name: '6.4 Тойрог ба өнцөг', slugs: ['toirog-ongog-1-8.html', 'toirog-ongog-2-8.html'] },
          { name: '6.5 Төсөөтэй дүрсүүд', slugs: ['tosootoi-1-8.html', 'tosootoi-2-8.html'] }
        ];
        for (const sub of subs8f) {
          let sgr = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='8-р анги' AND name=$1`, [sub.name]);
          let sid;
          if (sgr.rows.length) { sid = Number(sgr.rows[0].id); }
          else {
            const mx = await pool.query(`SELECT COALESCE(MAX(pos),0)+1 AS p FROM ws_subgroups WHERE grade='8-р анги'`);
            const ins = await pool.query(`INSERT INTO ws_subgroups (grade, name, pos, parent_id) VALUES ('8-р анги',$1,$2,$3) RETURNING id`, [sub.name, mx.rows[0].p, pid8f]);
            sid = Number(ins.rows[0].id);
          }
          for (const slug of sub.slugs) {
            await pool.query(`INSERT INTO ws_place (grp, slug, kind) VALUES ($1,$2,'add') ON CONFLICT DO NOTHING`, ['sg:' + sid, slug]);
        ...
        await pool.query(`INSERT INTO ws_settings (skey, sval) VALUES ('place_g8ch6_v1','1') ON CONFLICT (skey) DO UPDATE SET sval='1'`);
      }
      // ── Нэг удаагийн: 8-р анги I ба III бүлэгт хосолсон (олон үйлдэлтэй) илэрхийллийн хуудас ──
      const cf8x = await pool.query(`SELECT sval FROM ws_settings WHERE skey='place_g8extra_v1'`);
      if (!cf8x.rows.length) {
        const groups8x = [
          { sub: '1.3 Рационал тооны үйлдэл', chap: 'I БҮЛЭГ%', chapFull: 'I БҮЛЭГ. ТООН ОЛОНЛОГ, ЗЭРЭГ ЯЗГУУР',
            slugs: ['rats-holimog-1-8.html', 'rats-holimog-2-8.html'] },
          { sub: '3.1 Олонлог, түүн дээрх үйлдэл', chap: 'III БҮЛЭГ%', chapFull: 'III БҮЛЭГ. МАГАДЛАЛ',
            slugs: ['olonlog-holimog-1-8.html', 'olonlog-holimog-2-8.html'] },
        ];
        for (const gg of groups8x) {
          let sgr = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='8-р анги' AND name=$1`, [gg.sub]);
          let sid;
          if (sgr.rows.length) { sid = Number(sgr.rows[0].id); }
          else {
            // дэд бүлэг байхгүй бол эцэг бүлгийг олж (эсвэл үүсгэж) шинээр нэмнэ
            let par = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='8-р анги' AND name LIKE $1 AND parent_id IS NULL ORDER BY id LIMIT 1`, [gg.chap]);
            let pid;
            if (par.rows.length) { pid = Number(par.rows[0].id); }
            else {
              const mxP = await pool.query(`SELECT COALESCE(MAX(pos),0)+1 AS p FROM ws_subgroups WHERE grade='8-р анги'`);
              const insP = await pool.query(`INSERT INTO ws_subgroups (grade, name, pos, parent_id) VALUES ('8-р анги',$1,$2,NULL) RETURNING id`, [gg.chapFull, mxP.rows[0].p]);
              pid = Number(insP.rows[0].id);
            }
            const mx = await pool.query(`SELECT COALESCE(MAX(pos),0)+1 AS p FROM ws_subgroups WHERE grade='8-р анги'`);
            const ins = await pool.query(`INSERT INTO ws_subgroups (grade, name, pos, parent_id) VALUES ('8-р анги',$1,$2,$3) RETURNING id`, [gg.sub, mx.rows[0].p, pid]);
            sid = Number(ins.rows[0].id);
          }
          for (const slug of gg.slugs) {
            await pool.query(`INSERT INTO ws_place (grp, slug, kind) VALUES ($1,$2,'add') ON CONFLICT DO NOTHING`, ['sg:' + sid, slug]);
          }
        }
        await pool.query(`INSERT INTO ws_settings (skey, sval) VALUES ('place_g8extra_v1','1') ON CONFLICT (skey) DO UPDATE SET sval='1'`);
      }
      // ── Нэг удаагийн: 7-р анги I БҮЛЭГ ──
      const cf_g7c1 = await pool.query(`SELECT sval FROM ws_settings WHERE skey='place_g7c1_v1'`);
      if (!cf_g7c1.rows.length) {
        let par_g7c1 = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='7-р анги' AND name LIKE 'I БҮЛЭГ%' AND parent_id IS NULL ORDER BY id LIMIT 1`);
        let pid_g7c1;
        ...
        const subs_g7c1 = [
          { name: "1.1 Тооны модул", slugs: ["toony-modul-1-7.html", "toony-modul-2-7.html"] },
          { name: "1.2 Бүхэл тооны нэмэх, хасах үйлдэл", slugs: ["buhel-nemeh-hasah-1-7.html", "buhel-nemeh-hasah-2-7.html"] },
          { name: "1.3 Бүхэл тооны үржүүлэх, хуваах үйлдэл", slugs: ["buhel-urjih-huvaah-1-7.html", "buhel-urjih-huvaah-2-7.html"] },
          { name: "1.4 Куб зэрэг ба куб язгуур", slugs: ["kub-zereg-1-7.html", "kub-zereg-2-7.html"] },
          { name: "1.5 Анхны тоон үржигдэхүүнд задлах", slugs: ["anhny-too-zadlah-7.html"] },
          { name: "1.6 ХИЕХ ба ХБЕХ", slugs: ["hieh-hbeh-1-7.html", "hieh-hbeh-2-7.html"] },
        ];
        for (const sub of subs_g7c1) {
          let sgr = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='7-р анги' AND name=$1`, [sub.name]);
          let sid;
          if (sgr.rows.length) { sid = Number(sgr.rows[0].id); }
          else {
            const mx = await pool.query(`SELECT COALESCE(MAX(pos),0)+1 AS p FROM ws_subgroups WHERE grade='7-р анги'`);
            const ins = await pool.query(`INSERT INTO ws_subgroups (grade, name, pos, parent_id) VALUES ('7-р анги',$1,$2,$3) RETURNING id`, [sub.name, mx.rows[0].p, pid_g7c1]);
            sid = Number(ins.rows[0].id);
          }
          for (const slug of sub.slugs) {
            await pool.query(`INSERT INTO ws_place (grp, slug, kind) VALUES ($1,$2,'add') ON CONFLICT DO NOTHING`, ['sg:' + sid, slug]);
        ...
        await pool.query(`INSERT INTO ws_settings (skey, sval) VALUES ('place_g7c1_v1','1') ON CONFLICT (skey) DO UPDATE SET sval='1'`);
      }

      // ── Нэг удаагийн: 7-р анги II БҮЛЭГ ──
      const cf_g7c2 = await pool.query(`SELECT sval FROM ws_settings WHERE skey='place_g7c2_v1'`);
      if (!cf_g7c2.rows.length) {
        let par_g7c2 = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='7-р анги' AND name LIKE 'II БҮЛЭГ%' AND parent_id IS NULL ORDER BY id LIMIT 1`);
        let pid_g7c2;
        ...
        const subs_g7c2 = [
          { name: "2.1 10-ын натурал илтгэгчтэй зэрэг", slugs: ["arawtiin-zereg-uildel-7.html"] },
          { name: "2.2 Аравтын бутархайг жиших, эрэмбэлэх", slugs: ["arawtiin-butarhai-jishih-7.html"] },
          { name: "2.3 Тоог тоймлох", slugs: ["toog-toimloh-7.html"] },
          { name: "2.4 Хялбар аргаар тооцоолох", slugs: ["holimog-uildel-butarhai-7.html"] },
          { name: "2.5 Энгийн бутархайг хураах, жиших", slugs: ["engiin-butarhai-huraah-jishih-7.html"] },
          { name: "2.6 Энгийн бутархайн нэмэх, хасах", slugs: ["engiin-butarhai-nemeh-hasah-7.html"] },
          { name: "2.7 Энгийн бутархайн үржүүлэх, хуваах", slugs: ["engiin-butarhai-urjih-huvaah-7.html"] },
          { name: "2.8 Бутархай ба процент харилцан шилжүүлэх", slugs: ["butarhai-protsent-1-7.html", "butarhai-protsent-2-7.html"] },
          { name: "2.9 Хөдөлгөөн ба ажлын бодлого", slugs: ["hodolgoon-ajil-bodlogo-7.html"] },
          { name: "2.10 Процентын бодлого", slugs: ["protsentiin-bodlogo-7.html"] },
          { name: "2.11 Өсөлт, бууралтын процент", slugs: ["osolt-buuraltiin-protsent-7.html"] },
        ];
        for (const sub of subs_g7c2) {
          let sgr = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='7-р анги' AND name=$1`, [sub.name]);
          let sid;
          if (sgr.rows.length) { sid = Number(sgr.rows[0].id); }
          else {
            const mx = await pool.query(`SELECT COALESCE(MAX(pos),0)+1 AS p FROM ws_subgroups WHERE grade='7-р анги'`);
            const ins = await pool.query(`INSERT INTO ws_subgroups (grade, name, pos, parent_id) VALUES ('7-р анги',$1,$2,$3) RETURNING id`, [sub.name, mx.rows[0].p, pid_g7c2]);
            sid = Number(ins.rows[0].id);
          }
          for (const slug of sub.slugs) {
            await pool.query(`INSERT INTO ws_place (grp, slug, kind) VALUES ($1,$2,'add') ON CONFLICT DO NOTHING`, ['sg:' + sid, slug]);
        ...
        await pool.query(`INSERT INTO ws_settings (skey, sval) VALUES ('place_g7c2_v1','1') ON CONFLICT (skey) DO UPDATE SET sval='1'`);
      }

      // ── Нэг удаагийн: 7-р анги III БҮЛЭГ ──
      const cf_g7c3 = await pool.query(`SELECT sval FROM ws_settings WHERE skey='place_g7c3_v1'`);
      if (!cf_g7c3.rows.length) {
        let par_g7c3 = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='7-р анги' AND name LIKE 'III БҮЛЭГ%' AND parent_id IS NULL ORDER BY id LIMIT 1`);
        let pid_g7c3;
        ...
        const subs_g7c3 = [
          { name: "3.1 Харьцаа ба түүний хялбар хэлбэр", slugs: ["haritsaa-hylbar-7.html"] },
          { name: "3.2 Пропорц ба үндсэн чанар", slugs: ["proports-1-7.html"] },
          { name: "3.3 Харьцааг процентоор илэрхийлэх", slugs: ["proports-2-7.html"] },
          { name: "3.4 Харьцаагаар хуваах", slugs: ["haritsaagaar-huvaah-7.html"] },
          { name: "3.5 Шууд пропорционал хамаарал", slugs: ["shuud-proports-1-7.html", "shuud-proports-2-7.html"] },
        ];
        for (const sub of subs_g7c3) {
          let sgr = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='7-р анги' AND name=$1`, [sub.name]);
          let sid;
          if (sgr.rows.length) { sid = Number(sgr.rows[0].id); }
          else {
            const mx = await pool.query(`SELECT COALESCE(MAX(pos),0)+1 AS p FROM ws_subgroups WHERE grade='7-р анги'`);
            const ins = await pool.query(`INSERT INTO ws_subgroups (grade, name, pos, parent_id) VALUES ('7-р анги',$1,$2,$3) RETURNING id`, [sub.name, mx.rows[0].p, pid_g7c3]);
            sid = Number(ins.rows[0].id);
          }
          for (const slug of sub.slugs) {
            await pool.query(`INSERT INTO ws_place (grp, slug, kind) VALUES ($1,$2,'add') ON CONFLICT DO NOTHING`, ['sg:' + sid, slug]);
        ...
        await pool.query(`INSERT INTO ws_settings (skey, sval) VALUES ('place_g7c3_v1','1') ON CONFLICT (skey) DO UPDATE SET sval='1'`);
      }

      // ── Нэг удаагийн: 7-р анги IV БҮЛЭГ ──
      const cf_g7c4 = await pool.query(`SELECT sval FROM ws_settings WHERE skey='place_g7c4_v1'`);
      if (!cf_g7c4.rows.length) {
        let par_g7c4 = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='7-р анги' AND name LIKE 'IV БҮЛЭГ%' AND parent_id IS NULL ORDER BY id LIMIT 1`);
        let pid_g7c4;
        ...
        const subs_g7c4 = [
          { name: "4.1 Эсрэг үзэгдлийн магадлал", slugs: ["esreg-uzegdel-1-7.html", "esreg-uzegdel-2-7.html"] },
          { name: "4.2 Нэгдсэн туршилт (шоо, зоос, шагай)", slugs: ["hoyor-shoo-1-7.html", "hoyor-shoo-2-7.html", "zoos-negdsen-turshilt-7.html"] },
          { name: "4.3 Санамсаргүй сонголтын магадлал", slugs: ["sanamsargui-too-1-7.html", "sanamsargui-too-2-7.html"] },
        ];
        for (const sub of subs_g7c4) {
          let sgr = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='7-р анги' AND name=$1`, [sub.name]);
          let sid;
          if (sgr.rows.length) { sid = Number(sgr.rows[0].id); }
          else {
            const mx = await pool.query(`SELECT COALESCE(MAX(pos),0)+1 AS p FROM ws_subgroups WHERE grade='7-р анги'`);
            const ins = await pool.query(`INSERT INTO ws_subgroups (grade, name, pos, parent_id) VALUES ('7-р анги',$1,$2,$3) RETURNING id`, [sub.name, mx.rows[0].p, pid_g7c4]);
            sid = Number(ins.rows[0].id);
          }
          for (const slug of sub.slugs) {
            await pool.query(`INSERT INTO ws_place (grp, slug, kind) VALUES ($1,$2,'add') ON CONFLICT DO NOTHING`, ['sg:' + sid, slug]);
        ...
        await pool.query(`INSERT INTO ws_settings (skey, sval) VALUES ('place_g7c4_v1','1') ON CONFLICT (skey) DO UPDATE SET sval='1'`);
      }

      // ── Нэг удаагийн: 7-р анги V БҮЛЭГ ──
      const cf_g7c5 = await pool.query(`SELECT sval FROM ws_settings WHERE skey='place_g7c5_v1'`);
      if (!cf_g7c5.rows.length) {
        let par_g7c5 = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='7-р анги' AND name LIKE 'V БҮЛЭГ%' AND parent_id IS NULL ORDER BY id LIMIT 1`);
        let pid_g7c5;
        ...
        const subs_g7c5 = [
          { name: "5.1 Алгебрын илэрхийлэл хялбарлах", slugs: ["tosootoi-gishuud-7.html", "haalt-zadlah-7.html"] },
          { name: "5.2 Натурал илтгэгчтэй зэргийн чанарууд", slugs: ["zergiin-chanar-7.html"] },
          { name: "5.3 Нэг гишүүнтүүдийг үржүүлэх, хуваах", slugs: ["neg-gishuunt-1-7.html", "neg-gishuunt-2-7.html"] },
          { name: "5.4 Нэг гишүүнтээр үржүүлэх, ерөнхий үржигдэхүүн гаргах", slugs: ["gishuunchlen-urjuuleh-7.html"] },
          { name: "5.5 Шугаман тэгшитгэл бодох", slugs: ["shugaman-tegshitgel-1-7.html", "shugaman-tegshitgel-2-7.html"] },
          { name: "5.6 Тэгшитгэл зохиож бодлого бодох", slugs: ["tegshitgel-bodlogo-7.html"] },
        ];
        for (const sub of subs_g7c5) {
          let sgr = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='7-р анги' AND name=$1`, [sub.name]);
          let sid;
          if (sgr.rows.length) { sid = Number(sgr.rows[0].id); }
          else {
            const mx = await pool.query(`SELECT COALESCE(MAX(pos),0)+1 AS p FROM ws_subgroups WHERE grade='7-р анги'`);
            const ins = await pool.query(`INSERT INTO ws_subgroups (grade, name, pos, parent_id) VALUES ('7-р анги',$1,$2,$3) RETURNING id`, [sub.name, mx.rows[0].p, pid_g7c5]);
            sid = Number(ins.rows[0].id);
          }
          for (const slug of sub.slugs) {
            await pool.query(`INSERT INTO ws_place (grp, slug, kind) VALUES ($1,$2,'add') ON CONFLICT DO NOTHING`, ['sg:' + sid, slug]);
        ...
        await pool.query(`INSERT INTO ws_settings (skey, sval) VALUES ('place_g7c5_v1','1') ON CONFLICT (skey) DO UPDATE SET sval='1'`);
      }

      // ── Нэг удаагийн: 7-р анги VI БҮЛЭГ ──
      const cf_g7c6 = await pool.query(`SELECT sval FROM ws_settings WHERE skey='place_g7c6_v1'`);
      if (!cf_g7c6.rows.length) {
        let par_g7c6 = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='7-р анги' AND name LIKE 'VI БҮЛЭГ%' AND parent_id IS NULL ORDER BY id LIMIT 1`);
        let pid_g7c6;
        ...
        const subs_g7c6 = [
          { name: "6.1 Тоон дараалал ба ерөнхий гишүүн", slugs: ["toon-daraalal-1-7.html", "toon-daraalal-2-7.html", "eronhii-gishuun-7.html"] },
          { name: "6.2 Арифметик прогресс", slugs: ["arifmetik-progress-7.html", "arifmetik-bodlogo-1-7.html", "arifmetik-bodlogo-2-7.html"] },
          { name: "6.3 Шугаман функц, шулууны тэгшитгэл", slugs: ["shugaman-funkts-7.html"] },
          { name: "6.4 Шугаман функцийн график, шууд пропорционал", slugs: ["funkts-huesnegt-7.html", "shuud-proportsional-7.html"] },
        ];
        for (const sub of subs_g7c6) {
          let sgr = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='7-р анги' AND name=$1`, [sub.name]);
          let sid;
          if (sgr.rows.length) { sid = Number(sgr.rows[0].id); }
          else {
            const mx = await pool.query(`SELECT COALESCE(MAX(pos),0)+1 AS p FROM ws_subgroups WHERE grade='7-р анги'`);
            const ins = await pool.query(`INSERT INTO ws_subgroups (grade, name, pos, parent_id) VALUES ('7-р анги',$1,$2,$3) RETURNING id`, [sub.name, mx.rows[0].p, pid_g7c6]);
            sid = Number(ins.rows[0].id);
          }
          for (const slug of sub.slugs) {
            await pool.query(`INSERT INTO ws_place (grp, slug, kind) VALUES ($1,$2,'add') ON CONFLICT DO NOTHING`, ['sg:' + sid, slug]);
      ...
      // ── Нэг удаагийн: 11-р анги ЖИШИГ ДААЛГАВАР (бодолттой) ──
      const cf_g11ex = await pool.query(`SELECT sval FROM ws_settings WHERE skey='place_g11exam_v1'`);
      if (!cf_g11ex.rows.length) {
        let sg_g11ex = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='11-р анги' AND name=$1`, ["ЖИШИГ ДААЛГАВАР — бодолт хавсаргасан"]);
        let sid_g11ex;
        if (sg_g11ex.rows.length) { sid_g11ex = Number(sg_g11ex.rows[0].id); }
        else {
          const mx = await pool.query(`SELECT COALESCE(MAX(pos),0)+1 AS p FROM ws_subgroups WHERE grade='11-р анги'`);
          const ins = await pool.query(`INSERT INTO ws_subgroups (grade, name, pos, parent_id) VALUES ('11-р анги',$1,$2,NULL) RETURNING id`, ["ЖИШИГ ДААЛГАВАР — бодолт хавсаргасан", mx.rows[0].p]);
          sid_g11ex = Number(ins.rows[0].id);
        }
        const slugs_g11ex = ["jishig-daalgavar-1-11.html"];
        for (const slug of slugs_g11ex) {
          await pool.query(`INSERT INTO ws_place (grp, slug, kind) VALUES ($1,$2,'add') ON CONFLICT DO NOTHING`, ['sg:' + sid_g11ex, slug]);
        }
        await pool.query(`INSERT INTO ws_settings (skey, sval) VALUES ('place_g11exam_v1','1') ON CONFLICT (skey) DO UPDATE SET sval='1'`);
      }
      ...
      if (!cf_g6c1.rows.length) {
        let par_g6c1 = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='6-р анги' AND name LIKE 'I БҮЛЭГ%' AND parent_id IS NULL ORDER BY id LIMIT 1`);
        let pid_g6c1;
        if (par_g6c1.rows.length) { pid_g6c1 = Number(par_g6c1.rows[0].id); }
        else {
          const mxP = await pool.query(`SELECT COALESCE(MAX(pos),0)+1 AS p FROM ws_subgroups WHERE grade='6-р анги'`);
          const insP = await pool.query(`INSERT INTO ws_subgroups (grade, name, pos, parent_id) VALUES ('6-р анги',$2,$1,NULL) RETURNING id`, [mxP.rows[0].p, "I БҮЛЭГ. БҮХЭЛ ТООН ОЛОНЛОГ, ЗЭРЭГ, ЯЗГУУР"]);
          pid_g6c1 = Number(insP.rows[0].id);
        }
        const subs_g6c1 = [
          { name: "1.1 Бүхэл тоонуудыг жиших", slugs: ["buhel-too-jish-1-6.html", "buhel-too-jish-2-6.html", "buhel-too-jish-3-6.html", "buhel-too-jish-4-6.html"] },
          { name: "1.2 Бүхэл тооны нэмэх, хасах үйлдэл", slugs: ["buhel-nemeh-hasah-1-6.html", "buhel-nemeh-hasah-2-6.html", "buhel-nemeh-hasah-3-6.html", "buhel-nemeh-hasah-4-6.html", "buhel-nemeh-hasah-5-6.html"] },
          { name: "1.3 Бүхэл тооны үржүүлэх, хуваах үйлдэл", slugs: ["buhel-urjih-huvaah-1-6.html", "buhel-urjih-huvaah-2-6.html", "buhel-urjih-huvaah-3-6.html", "buhel-urjih-huvaah-4-6.html"] },
          { name: "1.4 4, 6 ба 8-д хуваагдах тооны шинж", slugs: ["huvaagdah-shinj-1-6.html", "huvaagdah-shinj-2-6.html", "huvaagdah-shinj-3-6.html"] },
          { name: "1.5 Ерөнхий хуваагч ба хамгийн их ерөнхий хуваагч", slugs: ["erunhii-huvaagch-1-6.html", "erunhii-huvaagch-2-6.html", "erunhii-huvaagch-3-6.html"] },
          { name: "1.6 Ерөнхий хуваагдагч ба хамгийн бага ерөнхий хуваагдагч", slugs: ["erunhii-huvaagdagch-1-6.html", "erunhii-huvaagdagch-2-6.html", "erunhii-huvaagdagch-3-6.html"] },
          { name: "1.7 Анхны тоо", slugs: ["anhny-too-1-6.html", "anhny-too-2-6.html", "anhny-too-3-6.html"] },
          { name: "1.8 Тооны квадрат зэрэг ба квадрат язгуур", slugs: ["kvadrat-zereg-yazguur-1-6.html", "kvadrat-zereg-yazguur-2-6.html", "kvadrat-zereg-yazguur-3-6.html", "kvadrat-zereg-yazguur-4-6.html", "kvadrat-zereg-yazguur-5-6.html"] },
          { name: "1.9 Хэллэг", slugs: ["helleg-1-6.html", "helleg-2-6.html", "helleg-3-6.html"] },
          { name: "1.10 Олонлог", slugs: ["olonlog-1-6.html", "olonlog-2-6.html", "olonlog-3-6.html", "olonlog-4-6.html"] },
          { name: "1.11 Сонгон судлах — хүнд даалгавар", slugs: ["buhel-zereg-songon-1-6.html", "buhel-zereg-songon-2-6.html", "buhel-zereg-songon-3-6.html"] },
        ];
        for (const sub of subs_g6c1) {
          let sgr = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='6-р анги' AND name=$1`, [sub.name]);
          let sid;
          if (sgr.rows.length) { sid = Number(sgr.rows[0].id); }
          else {
            const mx = await pool.query(`SELECT COALESCE(MAX(pos),0)+1 AS p FROM ws_subgroups WHERE grade='6-р анги'`);
            const ins = await pool.query(`INSERT INTO ws_subgroups (grade, name, pos, parent_id) VALUES ('6-р анги',$1,$2,$3) RETURNING id`, [sub.name, mx.rows[0].p, pid_g6c1]);
            sid = Number(ins.rows[0].id);
          }
          for (const slug of sub.slugs) {
            await pool.query(`INSERT INTO ws_place (grp, slug, kind) VALUES ($1,$2,'add') ON CONFLICT DO NOTHING`, ['sg:' + sid, slug]);
        ...
        await pool.query(`INSERT INTO ws_settings (skey, sval) VALUES ('place_g6c1_v1','1') ON CONFLICT (skey) DO UPDATE SET sval='1'`);
      }

      // ── Нэг удаагийн: 6-р анги II БҮЛЭГ ──
      const cf_g6c2 = await pool.query(`SELECT sval FROM ws_settings WHERE skey='place_g6c2_v1'`);
      if (!cf_g6c2.rows.length) {
        let par_g6c2 = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='6-р анги' AND name LIKE 'II БҮЛЭГ%' AND parent_id IS NULL ORDER BY id LIMIT 1`);
        let pid_g6c2;
        if (par_g6c2.rows.length) { pid_g6c2 = Number(par_g6c2.rows[0].id); }
        else {
          const mxP = await pool.query(`SELECT COALESCE(MAX(pos),0)+1 AS p FROM ws_subgroups WHERE grade='6-р анги'`);
          const insP = await pool.query(`INSERT INTO ws_subgroups (grade, name, pos, parent_id) VALUES ('6-р анги',$2,$1,NULL) RETURNING id`, [mxP.rows[0].p, "II БҮЛЭГ. АРАВТЫН БА ЭНГИЙН БУТАРХАЙ, ПРОЦЕНТ"]);
          pid_g6c2 = Number(insP.rows[0].id);
        }
        const subs_g6c2 = [
          { name: "2.1 Тоог жиших, эрэмбэлэх", slugs: ["aravt-jishih-eremblel-1-6.html", "aravt-jishih-eremblel-2-6.html", "aravt-jishih-eremblel-3-6.html", "aravt-jishih-eremblel-4-6.html"] },
          { name: "2.2 Тоог тоймлох", slugs: ["toog-toymloh-1-6.html", "toog-toymloh-2-6.html", "toog-toymloh-3-6.html", "toog-toymloh-4-6.html"] },
          { name: "2.3 Аравтын бутархайн нэмэх, хасах үйлдэл", slugs: ["arav-nemeh-hasah-1-6.html", "arav-nemeh-hasah-2-6.html", "arav-nemeh-hasah-3-6.html", "arav-nemeh-hasah-4-6.html", "arav-nemeh-hasah-5-6.html"] },
          { name: "2.4 Аравтын бутархайг бүхэл тоогоор үржүүлэх, бүхэл тоонд хуваах", slugs: ["arav-buhel-urjih-1-6.html", "arav-buhel-urjih-2-6.html", "arav-buhel-urjih-3-6.html", "arav-buhel-urjih-4-6.html", "arav-buhel-urjih-5-6.html"] },
          { name: "2.5 Аравтын бутархайн үржүүлэх, хуваах үйлдэл", slugs: ["arav-urjih-huvaah-1-6.html", "arav-urjih-huvaah-2-6.html", "arav-urjih-huvaah-3-6.html", "arav-urjih-huvaah-4-6.html", "arav-urjih-huvaah-5-6.html", "arav-urjih-huvaah-6-6.html", "arav-urjih-huvaah-7-6.html"] },
          { name: "2.6 Хялбар аргаар тооцоолох", slugs: ["hyalbar-tootsoolol-1-6.html", "hyalbar-tootsoolol-2-6.html", "hyalbar-tootsoolol-3-6.html"] },
          { name: "2.7 Энгийн бутархайг хураах, жиших", slugs: ["engiin-butarhai-hurah-1-6.html", "engiin-butarhai-hurah-2-6.html", "engiin-butarhai-hurah-3-6.html", "engiin-butarhai-hurah-4-6.html"] },
          { name: "2.8 Энгийн бутархайн нэмэх, хасах үйлдэл", slugs: ["engiin-butarhai-nemeh-1-6.html", "engiin-butarhai-nemeh-2-6.html", "engiin-butarhai-nemeh-3-6.html", "engiin-butarhai-nemeh-4-6.html", "engiin-butarhai-nemeh-5-6.html"] },
          { name: "2.9 Энгийн бутархайн үржүүлэх үйлдэл", slugs: ["engiin-butarhai-urjuuleh-1-6.html", "engiin-butarhai-urjuuleh-2-6.html", "engiin-butarhai-urjuuleh-3-6.html", "engiin-butarhai-urjuuleh-4-6.html"] },
          { name: "2.10 Энгийн бутархайн хуваах үйлдэл", slugs: ["engiin-butarhai-huvaah-1-6.html", "engiin-butarhai-huvaah-2-6.html", "engiin-butarhai-huvaah-3-6.html", "engiin-butarhai-huvaah-4-6.html"] },
          { name: "2.11 Процент", slugs: ["protsent-1-6.html", "protsent-2-6.html", "protsent-3-6.html", "protsent-4-6.html", "protsent-5-6.html"] },
          { name: "2.12 Сонгон судлах — хүнд даалгавар", slugs: ["butarhai-protsent-songon-1-6.html", "butarhai-protsent-songon-2-6.html", "butarhai-protsent-songon-3-6.html"] },
        ];
        for (const sub of subs_g6c2) {
          let sgr = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='6-р анги' AND name=$1`, [sub.name]);
          let sid;
          if (sgr.rows.length) { sid = Number(sgr.rows[0].id); }
          else {
            const mx = await pool.query(`SELECT COALESCE(MAX(pos),0)+1 AS p FROM ws_subgroups WHERE grade='6-р анги'`);
            const ins = await pool.query(`INSERT INTO ws_subgroups (grade, name, pos, parent_id) VALUES ('6-р анги',$1,$2,$3) RETURNING id`, [sub.name, mx.rows[0].p, pid_g6c2]);
            sid = Number(ins.rows[0].id);
          }
          for (const slug of sub.slugs) {
            await pool.query(`INSERT INTO ws_place (grp, slug, kind) VALUES ($1,$2,'add') ON CONFLICT DO NOTHING`, ['sg:' + sid, slug]);
        ...
        await pool.query(`INSERT INTO ws_settings (skey, sval) VALUES ('place_g6c2_v1','1') ON CONFLICT (skey) DO UPDATE SET sval='1'`);
      }

      // ── Нэг удаагийн: 6-р анги III БҮЛЭГ ──
      const cf_g6c3 = await pool.query(`SELECT sval FROM ws_settings WHERE skey='place_g6c3_v1'`);
      if (!cf_g6c3.rows.length) {
        let par_g6c3 = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='6-р анги' AND name LIKE 'III БҮЛЭГ%' AND parent_id IS NULL ORDER BY id LIMIT 1`);
        let pid_g6c3;
        if (par_g6c3.rows.length) { pid_g6c3 = Number(par_g6c3.rows[0].id); }
        else {
          const mxP = await pool.query(`SELECT COALESCE(MAX(pos),0)+1 AS p FROM ws_subgroups WHERE grade='6-р анги'`);
          const insP = await pool.query(`INSERT INTO ws_subgroups (grade, name, pos, parent_id) VALUES ('6-р анги',$2,$1,NULL) RETURNING id`, [mxP.rows[0].p, "III БҮЛЭГ. ХАРЬЦАА, ПРОПОРЦ"]);
          pid_g6c3 = Number(insP.rows[0].id);
        }
        const subs_g6c3 = [
          { name: "3.1 Тоог өгсөн харьцаагаар хуваах", slugs: ["haritsaagaar-huvaah-1-6.html", "haritsaagaar-huvaah-2-6.html", "haritsaagaar-huvaah-3-6.html", "haritsaagaar-huvaah-4-6.html", "haritsaagaar-huvaah-5-6.html"] },
          { name: "3.2 Пропорцын үндсэн чанар", slugs: ["proports-undsen-chanar-1-6.html", "proports-undsen-chanar-2-6.html", "proports-undsen-chanar-3-6.html", "proports-undsen-chanar-4-6.html"] },
          { name: "3.3 Шууд пропорционал хамаарал", slugs: ["shuud-proportsional-1-6.html", "shuud-proportsional-2-6.html", "shuud-proportsional-3-6.html"] },
          { name: "3.4 Сонгон судлах — хүнд даалгавар", slugs: ["haritsaa-songon-1-6.html", "haritsaa-songon-2-6.html"] },
        ];
        for (const sub of subs_g6c3) {
          let sgr = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='6-р анги' AND name=$1`, [sub.name]);
          let sid;
          if (sgr.rows.length) { sid = Number(sgr.rows[0].id); }
          else {
            const mx = await pool.query(`SELECT COALESCE(MAX(pos),0)+1 AS p FROM ws_subgroups WHERE grade='6-р анги'`);
            const ins = await pool.query(`INSERT INTO ws_subgroups (grade, name, pos, parent_id) VALUES ('6-р анги',$1,$2,$3) RETURNING id`, [sub.name, mx.rows[0].p, pid_g6c3]);
            sid = Number(ins.rows[0].id);
          }
          for (const slug of sub.slugs) {
            await pool.query(`INSERT INTO ws_place (grp, slug, kind) VALUES ($1,$2,'add') ON CONFLICT DO NOTHING`, ['sg:' + sid, slug]);
        ...
        await pool.query(`INSERT INTO ws_settings (skey, sval) VALUES ('place_g6c3_v1','1') ON CONFLICT (skey) DO UPDATE SET sval='1'`);
      }

      // ── Нэг удаагийн: 6-р анги IV БҮЛЭГ ──
      const cf_g6c4 = await pool.query(`SELECT sval FROM ws_settings WHERE skey='place_g6c4_v1'`);
      if (!cf_g6c4.rows.length) {
        let par_g6c4 = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='6-р анги' AND name LIKE 'IV БҮЛЭГ%' AND parent_id IS NULL ORDER BY id LIMIT 1`);
        let pid_g6c4;
        if (par_g6c4.rows.length) { pid_g6c4 = Number(par_g6c4.rows[0].id); }
        else {
          const mxP = await pool.query(`SELECT COALESCE(MAX(pos),0)+1 AS p FROM ws_subgroups WHERE grade='6-р анги'`);
          const insP = await pool.query(`INSERT INTO ws_subgroups (grade, name, pos, parent_id) VALUES ('6-р анги',$2,$1,NULL) RETURNING id`, [mxP.rows[0].p, "IV БҮЛЭГ. МАГАДЛАЛ"]);
          pid_g6c4 = Number(insP.rows[0].id);
        }
        const subs_g6c4 = [
          { name: "4.1 Үржвэр ба нийлбэрийн зарчим", slugs: ["urjveriin-zarchim-1-6.html", "urjveriin-zarchim-2-6.html", "tsifruudeer-too-zohioh-1-6.html", "tsifruudeer-too-zohioh-2-6.html", "toolokh-bodlogo-1-6.html", "toolokh-bodlogo-2-6.html"] },
          { name: "4.2 Үзэгдлийн магадлал", slugs: ["uzegdliin-magadlal-1-6.html", "uzegdliin-magadlal-2-6.html", "uzegdliin-magadlal-3-6.html", "uzegdliin-magadlal-4-6.html", "uzegdliin-magadlal-5-6.html"] },
          { name: "4.3 Сонгон судлах — хүнд даалгавар", slugs: ["magadlal-hund-1-6.html", "magadlal-hund-2-6.html", "magadlal-hund-3-6.html"] },
        ];
        for (const sub of subs_g6c4) {
          let sgr = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='6-р анги' AND name=$1`, [sub.name]);
          let sid;
          if (sgr.rows.length) { sid = Number(sgr.rows[0].id); }
          else {
            const mx = await pool.query(`SELECT COALESCE(MAX(pos),0)+1 AS p FROM ws_subgroups WHERE grade='6-р анги'`);
            const ins = await pool.query(`INSERT INTO ws_subgroups (grade, name, pos, parent_id) VALUES ('6-р анги',$1,$2,$3) RETURNING id`, [sub.name, mx.rows[0].p, pid_g6c4]);
            sid = Number(ins.rows[0].id);
          }
          for (const slug of sub.slugs) {
            await pool.query(`INSERT INTO ws_place (grp, slug, kind) VALUES ($1,$2,'add') ON CONFLICT DO NOTHING`, ['sg:' + sid, slug]);
        ...
        await pool.query(`INSERT INTO ws_settings (skey, sval) VALUES ('place_g6c4_v1','1') ON CONFLICT (skey) DO UPDATE SET sval='1'`);
      }

      // ── Нэг удаагийн: 6-р анги V БҮЛЭГ ──
      const cf_g6c5 = await pool.query(`SELECT sval FROM ws_settings WHERE skey='place_g6c5_v1'`);
      if (!cf_g6c5.rows.length) {
        let par_g6c5 = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='6-р анги' AND name LIKE 'V БҮЛЭГ%' AND parent_id IS NULL ORDER BY id LIMIT 1`);
        let pid_g6c5;
        if (par_g6c5.rows.length) { pid_g6c5 = Number(par_g6c5.rows[0].id); }
        else {
          const mxP = await pool.query(`SELECT COALESCE(MAX(pos),0)+1 AS p FROM ws_subgroups WHERE grade='6-р анги'`);
          const insP = await pool.query(`INSERT INTO ws_subgroups (grade, name, pos, parent_id) VALUES ('6-р анги',$2,$1,NULL) RETURNING id`, [mxP.rows[0].p, "V БҮЛЭГ. АЛГЕБРЫН ИЛЭРХИЙЛЭЛ, ТЭГШИТГЭЛ"]);
          pid_g6c5 = Number(insP.rows[0].id);
        }
        const subs_g6c5 = [
          { name: "5.1 Алгебрын илэрхийлэл зохиох, түүний утгыг олох", slugs: ["alg-ilerhiilel-zohioh-1-6.html", "alg-ilerhiilel-gishuun-6.html", "alg-ilerhiilel-utga-6.html", "alg-ilerhiilel-piramid-6.html", "alg-ilerhiilel-ugen-6.html"] },
          { name: "5.2 Алгебрын илэрхийллийг хялбарчлах", slugs: ["alg-hyalbarchlah-1-6.html", "alg-hyalbarchlah-2-6.html", "alg-hyalbarchlah-3-6.html", "alg-hyalbarchlah-4-6.html"] },
          { name: "5.3 Гишүүнчлэн үржүүлэх ба хаалтаас ерөнхий үржигдэхүүн гаргах", slugs: ["gishuunchlen-urjuuleh-1-6.html", "gishuunchlen-urjuuleh-2-6.html", "gishuunchlen-urjuuleh-3-6.html", "eronhii-urjigdehuun-1-6.html", "eronhii-urjigdehuun-2-6.html"] },
          { name: "5.4 Шугаман тэгшитгэл", slugs: ["shugaman-tegshitgel-1-6.html", "shugaman-tegshitgel-2-6.html", "tegshitgel-zohioh-1-6.html", "tegshitgel-zohioh-2-6.html", "tegshitgel-zohioh-3-6.html", "tegshitgel-zohioh-4-6.html", "tegshitgel-durs-6.html"] },
          { name: "5.5 Сонгон судлах — хүнд даалгавар", slugs: ["alg-tegshitgel-songon-1-6.html", "alg-tegshitgel-songon-2-6.html", "alg-tegshitgel-songon-3-6.html"] },
        ];
        for (const sub of subs_g6c5) {
          let sgr = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='6-р анги' AND name=$1`, [sub.name]);
          let sid;
          if (sgr.rows.length) { sid = Number(sgr.rows[0].id); }
          else {
            const mx = await pool.query(`SELECT COALESCE(MAX(pos),0)+1 AS p FROM ws_subgroups WHERE grade='6-р анги'`);
            const ins = await pool.query(`INSERT INTO ws_subgroups (grade, name, pos, parent_id) VALUES ('6-р анги',$1,$2,$3) RETURNING id`, [sub.name, mx.rows[0].p, pid_g6c5]);
            sid = Number(ins.rows[0].id);
          }
          for (const slug of sub.slugs) {
            await pool.query(`INSERT INTO ws_place (grp, slug, kind) VALUES ($1,$2,'add') ON CONFLICT DO NOTHING`, ['sg:' + sid, slug]);
        ...
        await pool.query(`INSERT INTO ws_settings (skey, sval) VALUES ('place_g6c5_v1','1') ON CONFLICT (skey) DO UPDATE SET sval='1'`);
      }

      // ── Нэг удаагийн: 6-р анги VI БҮЛЭГ ──
      const cf_g6c6 = await pool.query(`SELECT sval FROM ws_settings WHERE skey='place_g6c6_v1'`);
      if (!cf_g6c6.rows.length) {
        let par_g6c6 = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='6-р анги' AND name LIKE 'VI БҮЛЭГ%' AND parent_id IS NULL ORDER BY id LIMIT 1`);
        let pid_g6c6;
        if (par_g6c6.rows.length) { pid_g6c6 = Number(par_g6c6.rows[0].id); }
        else {
          const mxP = await pool.query(`SELECT COALESCE(MAX(pos),0)+1 AS p FROM ws_subgroups WHERE grade='6-р анги'`);
          const insP = await pool.query(`INSERT INTO ws_subgroups (grade, name, pos, parent_id) VALUES ('6-р анги',$2,$1,NULL) RETURNING id`, [mxP.rows[0].p, "VI БҮЛЭГ. ДАРААЛАЛ, ФУНКЦ"]);
          pid_g6c6 = Number(insP.rows[0].id);
        }
        const subs_g6c6 = [
          { name: "6.1 Дүрсэн дараалал", slugs: ["dursen-daraalal-1-6.html", "dursen-daraalal-2-6.html", "dursen-daraalal-3-6.html"] },
          { name: "6.2 Тоон дараалал", slugs: ["toon-daraalal-1-6.html", "toon-daraalal-2-6.html", "toon-daraalal-3-6.html", "toon-daraalal-4-6.html"] },
          { name: "6.3 Функц", slugs: ["funkts-1-6.html", "funkts-2-6.html", "funkts-3-6.html"] },
          { name: "6.4 Координатын хавтгай", slugs: ["koordinatyn-havtgai-1-6.html", "koordinatyn-havtgai-2-6.html", "koordinatyn-havtgai-3-6.html"] },
          { name: "6.5 Шулууны тэгшитгэл бичих", slugs: ["shuluuny-tegshitgel-1-6.html", "shuluuny-tegshitgel-2-6.html", "shuluuny-tegshitgel-3-6.html"] },
          { name: "6.6 Сонгон судлах — хүнд даалгавар", slugs: ["daraalal-funkts-hund-1-6.html", "daraalal-funkts-hund-2-6.html", "daraalal-funkts-hund-3-6.html"] },
        ];
        for (const sub of subs_g6c6) {
          let sgr = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='6-р анги' AND name=$1`, [sub.name]);
          let sid;
          if (sgr.rows.length) { sid = Number(sgr.rows[0].id); }
          else {
            const mx = await pool.query(`SELECT COALESCE(MAX(pos),0)+1 AS p FROM ws_subgroups WHERE grade='6-р анги'`);
            const ins = await pool.query(`INSERT INTO ws_subgroups (grade, name, pos, parent_id) VALUES ('6-р анги',$1,$2,$3) RETURNING id`, [sub.name, mx.rows[0].p, pid_g6c6]);
            sid = Number(ins.rows[0].id);
          }
          for (const slug of sub.slugs) {
            await pool.query(`INSERT INTO ws_place (grp, slug, kind) VALUES ($1,$2,'add') ON CONFLICT DO NOTHING`, ['sg:' + sid, slug]);
      ...
      const cf_g4c1 = await pool.query(`SELECT sval FROM ws_settings WHERE skey='place_g4c1_v1'`);
      if (!cf_g4c1.rows.length) {
        let par_g4c1 = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='4-р анги' AND name LIKE 'I БҮЛЭГ%' AND parent_id IS NULL ORDER BY id LIMIT 1`);
        let pid_g4c1;
        if (par_g4c1.rows.length) { pid_g4c1 = Number(par_g4c1.rows[0].id); }
        else {
          const mxP = await pool.query(`SELECT COALESCE(MAX(pos),0)+1 AS p FROM ws_subgroups WHERE grade='4-р анги'`);
          const insP = await pool.query(`INSERT INTO ws_subgroups (grade, name, pos, parent_id) VALUES ('4-р анги',$2,$1,NULL) RETURNING id`, [mxP.rows[0].p, "I БҮЛЭГ. САЯ ХҮРТЭЛХ ТОО, ҮРЖИХ, ХУВААХ"]);
          pid_g4c1 = Number(insP.rows[0].id);
        }
        const subs_g4c1 = [
          { name: "1.1 Давтах", slugs: ["davtah-1-4.html", "davtah-2-4.html"] },
          { name: "1.2 Сая хүртэлх тоог унших, бичих, жиших", slugs: ["saya-too-unsh-1-4.html", "saya-too-unsh-2-4.html"] },
          { name: "1.3 Сая хүртэлх тоог нэмэх, хасах", slugs: ["saya-too-nemeh-1-4.html", "saya-too-nemeh-2-4.html"] },
          { name: "1.4 Талбайг хэмжих, биетийн дэлгээс хийх", slugs: ["talbai-delgees-1-4.html", "talbai-delgees-2-4.html"] },
          { name: "1.5 Нийлбэрийг үржвэрээр бичих", slugs: ["niilber-urjver-1-4.html", "niilber-urjver-2-4.html"] },
          { name: "1.6 Нэг ба хоёр оронтой тоогоор үржүүлэх", slugs: ["urjuuleh-1-4.html", "urjuuleh-2-4.html", "urjuuleh-3-4.html"] },
          { name: "1.7 Нэг оронтой тоонд хуваах", slugs: ["neg-orontod-huvaah-1-4.html", "neg-orontod-huvaah-2-4.html", "neg-orontod-huvaah-3-4.html"] },
          { name: "1.8 Хоёр оронтой тоонд хуваах", slugs: ["hoyor-orontod-huvaah-1-4.html", "hoyor-orontod-huvaah-2-4.html", "hoyor-orontod-huvaah-3-4.html"] },
          { name: "1.9 Худалдаа хийхдээ бодлого бодох", slugs: ["hudaldaa-bodlogo-1-4.html", "hudaldaa-bodlogo-2-4.html"] },
          { name: "1.10 Эерэг, сөрөг тоог жиших", slugs: ["eereg-sorog-jish-1-4.html", "eereg-sorog-jish-2-4.html"] },
          { name: "1.11 Өгөгдөл цуглуулж диаграмм байгуулах", slugs: ["diagramm-1-4.html", "diagramm-2-4.html"] },
        ];
        for (const sub of subs_g4c1) {
          ...
          if (sgr.rows.length) { sid = Number(sgr.rows[0].id); }
            ...
            const ins = await pool.query(`INSERT INTO ws_subgroups (grade, name, pos, parent_id) VALUES ('4-р анги',$1,$2,$3) RETURNING id`, [sub.name, mx.rows[0].p, pid_g4c1]);
            sid = Number(ins.rows[0].id);
          }
          for (const slug of sub.slugs) {
            await pool.query(`INSERT INTO ws_place (grp, slug, kind) VALUES ($1,$2,'add') ON CONFLICT DO NOTHING`, ['sg:' + sid, slug]);
        ...
        await pool.query(`INSERT INTO ws_settings (skey, sval) VALUES ('place_g4c1_v1','1') ON CONFLICT (skey) DO UPDATE SET sval='1'`);
      }

      // ── Нэг удаагийн: 4-р анги II БҮЛЭГ ──
      const cf_g4c2 = await pool.query(`SELECT sval FROM ws_settings WHERE skey='place_g4c2_v1'`);
      if (!cf_g4c2.rows.length) {
        let par_g4c2 = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='4-р анги' AND name LIKE 'II БҮЛЭГ%' AND parent_id IS NULL ORDER BY id LIMIT 1`);
        let pid_g4c2;
        if (par_g4c2.rows.length) { pid_g4c2 = Number(par_g4c2.rows[0].id); }
        else {
          const mxP = await pool.query(`SELECT COALESCE(MAX(pos),0)+1 AS p FROM ws_subgroups WHERE grade='4-р анги'`);
          const insP = await pool.query(`INSERT INTO ws_subgroups (grade, name, pos, parent_id) VALUES ('4-р анги',$2,$1,NULL) RETURNING id`, [mxP.rows[0].p, "II БҮЛЭГ. БУТАРХАЙ, ХАРЬЦАА, ӨНЦӨГ"]);
          pid_g4c2 = Number(insP.rows[0].id);
        }
        const subs_g4c2 = [
          { name: "2.1 Энгийн бутархайг унших, бичих, жиших", slugs: ["engiin-butarhai-jish-1-4.html", "engiin-butarhai-jish-2-4.html"] },
          { name: "2.2 Энгийн бутархайг нэмэх, хасах", slugs: ["engiin-butarhai-nemeh-1-4.html", "engiin-butarhai-nemeh-2-4.html"] },
          { name: "2.3 Холимог бутархайг унших, бичих, жиших", slugs: ["holimog-butarhai-jish-1-4.html", "holimog-butarhai-jish-2-4.html"] },
          { name: "2.4 Холимог бутархайг нэмэх, хасах", slugs: ["holimog-butarhai-nemeh-1-4.html", "holimog-butarhai-nemeh-2-4.html"] },
          { name: "2.5 Хэд дахин багасгах, ихэсгэх", slugs: ["hed-dahin-1-4.html", "hed-dahin-2-4.html"] },
          { name: "2.6 Харьцаа", slugs: ["hartsaa-1-4.html", "hartsaa-2-4.html"] },
          { name: "2.7 Өнцөг хэмжих", slugs: ["ontsog-hemjih-1-4.html", "ontsog-hemjih-2-4.html"] },
          { name: "2.8 Гурвалжин ба олон өнцөгтийг ангилах", slugs: ["gurvaljin-olon-ontsogt-1-4.html", "gurvaljin-olon-ontsogt-2-4.html"] },
          { name: "2.9 Боломжийг тооцоолох", slugs: ["bolomj-1-4.html", "bolomj-2-4.html"] },
        ];
        for (const sub of subs_g4c2) {
          let sgr = await pool.query(`SELECT id FROM ws_subgroups WHERE grade='4-р анги' AND name=$1`, [sub.name]);
          let sid;
          if (sgr.rows.length) { sid = Number(sgr.rows[0].id); }
          else {
            const mx = await pool.query(`SELECT COALESCE(MAX(pos),0)+1 AS p FROM ws_subgroups WHERE grade='4-р анги'`);
            const ins = await pool.query(`INSERT INTO ws_subgroups (grade, name, pos, parent_id) VALUES ('4-р анги',$1,$2,$3) RETURNING id`, [sub.name, mx.rows[0].p, pid_g4c2]);
            sid = Number(ins.rows[0].id);
          }
          for (const slug of sub.slugs) {
            await pool.query(`INSERT INTO ws_place (grp, slug, kind) VALUES ($1,$2,'add') ON CONFLICT DO NOTHING`, ['sg:' + sid, slug]);
